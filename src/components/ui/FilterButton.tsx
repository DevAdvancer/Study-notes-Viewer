...
export function FilterButton({ status, isActive, onClick }: FilterButtonProps) {
  const getButtonStyle = () => {
    if (!isActive) return 'bg-[#0B2E33]/50 text-gray-400 hover:bg-[#0B2E33]';
    if (status === 'all') return 'bg-[#4F7C82] text-white';
    const statusColors = {
      'completed': 'text-green-500',
      'in-progress': 'text-yellow-500',
      'not-started': 'text-red-500'
    };
    return `bg-[#0B2E33] ${statusColors[status as Progress]}`;
  };

  // Get a highlight color based on status for the hover effect
  const getHighlightColor = (): string => {
    if (status === 'all') return 'rgba(79, 124, 130, 0.5)'; // teal/blue
